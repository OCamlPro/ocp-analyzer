...
module Make ( Tr : AltTypeTransducer.T) : S with type ty = Tr.t =
struct
  ...
  type pattern_matching =
    { mutable cases : (pattern list * ty lambda) list;
      args : (ty lambda * let_kind) list ;
  ...
  and pm_var_compiled =
    {inside : pm_half_compiled ; var_arg : ty lambda ; }
...
*)
  ...
  let rec raw_rec env : ty lambda -> ty lambda = function
    | Llet(Alias,x,ex, body) -> raw_rec ((x,raw_rec env ex)::env) body
    | Lvar id as l ->
      begin try List.assoc id env with
        | Not_found -> l
      end
    | Lprim (Pfield i,args, ty) ->
      Lprim (Pfield i, List.map (raw_rec env) args, ty)
    | Lconst _ as l -> l
    | Lstaticraise (i,args) ->
...
*)
  ...
  let rec extract_vars r p = match p.pat_desc with
    | Tpat_var (id, _) -> Lambda.IdentSet.add id r
    | Tpat_alias (p, id,_ ) ->
      extract_vars (Lambda.IdentSet.add id r) p
    | Tpat_tuple pats ->
  ...
  let pm_free_variables {cases=cases} =
    List.fold_right
      (fun (_,act) r -> Lambda.IdentSet.union (free_variables act) r)
      cases Lambda.IdentSet.empty
    ...
    let rec attempt seen = function
      | (q::qs,act_q) as cl::rem ->
        if is_or q then begin
          if compat p q then
            if
              Lambda.IdentSet.is_empty (extract_vars Lambda.IdentSet.empty p) &&
              Lambda.IdentSet.is_empty (extract_vars Lambda.IdentSet.empty q) &&
              equiv_pat p q
    ...
    | _  ->
      let rec do_cases = function
        | ({pat_desc=Tpat_or _} as orp::patl, action)::rem ->
          ...
          let vars =
            Lambda.IdentSet.elements
              (Lambda.IdentSet.inter
                 (extract_vars Lambda.IdentSet.empty orp)
...
(*
  ...
  let get_mod_field modname field =
    lazy (
      try
        ...
        in
        Lprim(Pfield p, [Lprim(Pgetglobal mod_ident, [], None)], None)
  ...
  *)
              ...
              Lifthenelse(
                ...
                Lifthenelse(
                  ...
                  Lprim(Pintcomp Ceq,
                        ...
                        None),
                  Lapply(force_fun, [varg], loc, Tr.no_type),
                  (* ... arg *)
                  varg))))
  ...
  let make_array_matching kind p def ctx = function
    ...
    | ((arg, mut) :: argl) ->
      ...
      let rec make_args pos =
        if pos >= len
        then argl
        else (Lprim(Parrayrefu kind, [arg; Lconst(Const_base(Const_int pos))], None),
  ...
  let rec do_tests_fail fail tst arg = function
    ...
    | (c, act)::rem ->
      Lifthenelse
        (Lprim (tst, [arg ; Lconst (Const_base c)], None),
  ...
  let make_test_sequence fail tst lt_tst arg const_lambda_list =
    ...
    and split_sequence const_lambda_list =
      ...
      Lifthenelse(Lprim(lt_tst,[arg; Lconst(Const_base (fst(List.hd list2)))], None),
  ...
  module SArg = struct
    type primitive = ty AltLambda.primitive
    ...
    type act = ty AltLambda.lambda

    let make_prim p args = Lprim (p,args,None)
    ...
    let make_isout h arg = Lprim (Pisout, [h ; arg], None)
...
*)
          ...
          (fun (ex, act) rem ->
             ...
             | Cstr_exception (path, _) ->
               Lifthenelse(Lprim(Pintcomp Ceq,
                                 [Lprim(Pfield 0, [arg], None); transl_path path],
                                 None),
                           act, rem)
             | _ -> assert false)
          tests default in
  ...
  let call_switcher_variant_constr fail arg int_lambda_list =
    ...
    Llet(Alias, v, Lprim(Pfield 0, [arg], None),
         call_switcher
           (fun i -> Lconst (Const_base (Const_int i)))
  ...
  let combine_variant row arg partial ctx def (tag_lambda_list, total1, pats) =
    ...
    let test_int_or_block arg if_int if_block =
      Lifthenelse(Lprim (Pisint, [arg], None), if_int, if_block) in
  ...
  let combine_array arg kind partial ctx def
    ...
    let lambda1 =
      ...
      bind
        Alias newvar (Lprim(Parraylength kind, [arg], None)) switch in
    lambda1, jumps_union local_jumps total1
...
*)
  ...
  *)
  ...
  let partial_function loc () =
    ...
    Lprim(Praise, [Lprim(Pmakeblock(0, Immutable,Tr.no_type),
...
*)
  ...
  let do_for_multiple_match loc paraml pat_act_list partial =
    ...
    let raise_num,pm1 =
      ...
      | Partial ->
        ...
        raise_num,
        { cases = List.map (fun (pat, act) -> ([pat], act)) pat_act_list;
          args = [Lprim(Pmakeblock(0, Immutable, Tr.no_type), paraml, None), Strict] ;
  ...
  let bind_opt (v,eo) k = match eo with
    | None -> k
    | Some e ->  AltLambda.bind Strict v e k
