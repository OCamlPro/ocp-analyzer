...
struct
  ...
  let transl_store_label_init glob size f arg =
    method_cache := Lprim(Pfield size, [Lprim(Pgetglobal glob, [], None)], None);
    let expr = f arg in
    let (size, expr) =
         ...
         Lsequence(
           Lprim(Psetfield(size, false),
                 [Lprim(Pgetglobal glob, [], None);
                  Lprim (Pccall prim_makearray, [int !method_count; int 0], None)],
                 None),
           expr))
    in
  ...
  let classes = ref []
  let method_ids = ref Lambda.IdentSet.empty
  ...
  let oo_wrap env req f x =
    if !wrapping then
      if !cache_required then f x else
        ...
        top_env := env;
        classes := [];
        method_ids := Lambda.IdentSet.empty;
        let lambda = f x in
        let lambda =
          List.fold_left
            (fun lambda id ->
               Llet(StrictOpt, id,
                    Lprim(Pmakeblock(0, Mutable,Tr.no_type),
