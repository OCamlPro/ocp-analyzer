...
and 'a lambda_switch =
  { sw_numconsts: int;
    sw_consts: (int * 'a lambda) list;
    sw_numblocks: int;
    sw_blocks: (int * 'a lambda) list;
    sw_failaction : 'a lambda option}
...
let rec same l1 l2 =
  ...
  | Lconst c1, Lconst c2 ->
      c1 = c2
  | Lapply(a1, bl1, _, _), Lapply(a2, bl2, _, _) ->
...
let rec transl_path = function
    Pident id ->
      if Ident.global id then Lprim(Pgetglobal id, [], None) else Lvar id
  | Pdot(p, s, pos) ->
...
let subst_lambda s lam =
  ...
  | Lconst sc as l -> l
  | Lapply(fn, args, loc, ty) -> Lapply(subst fn, List.map subst args, loc, ty)
  | Lfunction(kind, params, body) -> Lfunction(kind, params, subst body)
  | Llet(str, id, arg, body) -> Llet(str, id, subst arg, subst body)
  | Lletrec(decl, body) -> Lletrec(List.map subst_decl decl, subst body)
  | Lprim(p, args, ty) -> Lprim(p, List.map subst args, ty)
  | Lswitch(arg, sw) ->
      Lswitch(subst arg,
