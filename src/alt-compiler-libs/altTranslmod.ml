...
module Make (Tr : AltTypeTransducer.T)
    ...
    (Translclass : AltTranslclass.S with type ty = Tr.t)
  : S with type ty = Tr.t
=
struct
  ...
  and apply_coercion_field id (pos, cc) =
    apply_coercion cc (Lprim(Pfield pos, [Lvar id], None))
  ...
  let reorder_rec_bindings bindings =
    ...
    let fv = Array.map AltLambda.free_variables rhs in
      ...
      | Undefined ->
        if init.(i) = None then begin
          ...
          for j = 0 to num_bindings - 1 do
            if Lambda.IdentSet.mem id.(j) fv.(i) then emit_binding j
          done
        end;
  ...
  let eval_rec_bindings bindings cont =
    let rec bind_inits = function
      ...
      | (id, Some(loc, shape), rhs) :: rem ->
        Llet(Strict, id, Lapply(mod_prim "init_mod", [loc; shape], Location.none, Tr.no_type),
             bind_inits rem)
    ...
    | Tmod_apply(funct, arg, ccarg) ->
      oo_wrap mexp.mod_env true
        ...
        (Lapply(transl_module Tcoerce_none None funct,
                [transl_module ccarg None arg], mexp.mod_loc, Tr.no_type))
  ...
  let transl_store_structure glob map prims str =
    ...
    and store_ident id =
      try
        ...
        Lprim(Psetfield(pos, false), [Lprim(Pgetglobal glob, [], None); init_val], None)
      with Not_found ->
    ...
    and add_ident may_coerce id subst =
      try
        ...
        match cc with
          Tcoerce_none ->
          Ident.add id (Lprim(Pfield pos, [Lprim(Pgetglobal glob, [], None)], None)) subst
        | _ ->
  ...
  let close_toplevel_term lam =
    Lambda.IdentSet.fold (fun id l -> Llet(Strict, id, toploop_getvalue id, l))
  ...
  let get_component = function
    ...
    | Some id -> Lprim(Pgetglobal id, [], None)
  ...
  let transl_package component_names target_name coercion =
    let components =
      ...
      | _ ->
        assert false in
    Lprim(Psetglobal target_name, [Lprim(Pmakeblock(0, Immutable,Tr.no_type), components, None)], None)
