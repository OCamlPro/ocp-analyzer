...
module Make (Tr : AltTypeTransducer.T)
    (Matching : AltMatching.S with type ty = Tr.t)
    (Translobj : AltTranslobj.S with type ty = Tr.t)
    (Translcore : AltTranslcore.S with type ty = Tr.t) :
  S with type ty = Tr.t
=
struct
  ...
  let lapply func args loc tr =
    match func with
      Lapply(func', args', _, _) ->
      Lapply(func', args' @ args, loc, tr)
    | _ ->
      Lapply(func, args, loc, tr)

  let mkappl (func, args) = Lapply (func, args, Location.none, Tr.no_type);;
  ...
  let set_inst_var obj id expr =
    ...
    Lprim(Parraysetu kind, [Lvar obj; Lvar id; transl_exp expr], None)
  ...
  let rec build_object_init cl_table obj params inh_init obj_init cl =
    match cl.cl_desc with
      ...
      let env =
        match envs with None -> []
                      | Some envs -> [Lprim(Pfield (List.length inh_init + 1), [Lvar envs], None)]
      in
    ...
    | Tcl_apply (cl, oexprs) ->
      ...
      in
      (inh_init, transl_apply obj_init oexprs Location.none Tr.no_type)
  ...
  let output_methods tbl methods lam =
    ...
    | _ ->
      lsequence (mkappl(oo_prim "set_methods",
                        [Lvar tbl; Lprim(Pmakeblock(0,Immutable,Tr.no_type), methods, None)]))
        lam
      ...
      begin match inh_init with
          ...
          (inh_init,
           Llet (Strict, obj_init,
                 mkappl(Lprim(Pfield 1, [lpath], None), Lvar cla ::
                                                  if top then [Lprim(Pfield 3, [lpath], None)] else []),
  ...
  let rec get_class_meths cl =
    match cl.cl_desc with
      Tcl_structure cl ->
      Meths.fold (fun _ -> Lambda.IdentSet.add) cl.cstr_meths Lambda.IdentSet.empty
    | Tcl_ident _ -> Lambda.IdentSet.empty
    | Tcl_fun (_, _, _, cl, _)
    | Tcl_let (_, _, _, cl)
    | Tcl_apply (cl, _)
...
*)
  let rec transl_class_rebind obj_init cl vf =
    ...
    | Tcl_apply (cl, oexprs) ->
      let path, obj_init = transl_class_rebind obj_init cl vf in
      (path, transl_apply obj_init oexprs Location.none Tr.no_type)
    | Tcl_let (rec_flag, defs, vals, cl) ->
      let path, obj_init = transl_class_rebind obj_init cl vf in
  ...
  let transl_class_rebind ids cl vf =
    try
      ...
      let obj_init0 = lapply (Lvar obj_init) [Lvar self] Location.none Tr.no_type in
      ...
      if id then transl_path path else
        ...
        Llet(
          ...
          Llet(
            ...
            Lprim(Pmakeblock(0, Immutable,Tr.no_type),
  ...
  let const_path local = function
    ...
    | Lfunction (Curried, _, body) ->
      ...
      List.for_all (fun x -> not (Lambda.IdentSet.mem x fv)) local
    | p -> module_path p
  ...
  let rec builtin_meths self env env2 body =
    ...
    | Lapply(f, [arg; p], _, _) when const_path f && const_path p ->
      let s, args = conv arg in
      ("app_"^s^"_const", f :: args @ [p])
    | Lapply(f, [p; arg], _, _) when const_path f && const_path p ->
      let s, args = conv arg in
  ...
  let transl_class ids cl_id pub_meths cl vflag =
    ...
    if rebind <> lambda_unit then rebind else
      ...
      let subst env lam i0 new_ids' =
        ...
        (* prerr_ids "cl_id =" [cl_id]; prerr_ids "fv =" (Lambda.IdentSet.elements fv); *)
        let fv = List.fold_right Lambda.IdentSet.remove !new_ids' fv in
        ...
        method_ids :=
          Lambda.IdentSet.diff (Lambda.IdentSet.union (free_methods lam) !method_ids) meth_ids;
        (* prerr_ids "meth_ids =" (Lambda.IdentSet.elements meth_ids);
           prerr_ids "method_ids =" (Lambda.IdentSet.elements !method_ids); *)
        let new_ids = List.fold_right Lambda.IdentSet.add new_ids !method_ids in
        let fv = Lambda.IdentSet.inter fv new_ids in
        new_ids' := !new_ids' @ Lambda.IdentSet.elements fv;
      ...
      let msubst arr = function
          ...
          begin try
            ...
            with Not_found ->
              [lfunction (self :: args)
                 (if not (Lambda.IdentSet.mem env (free_variables body')) then body' else
                    Llet(Alias, env,
                         Lprim(Parrayrefu Paddrarray,
                               [Lvar self; Lvar env2], None), body'))]
          end
        | _ -> assert false
      in
      ...
      if top && ids = [] then llets (ltable cla (ldirect obj_init)) else
        ...
        and lbody fv =
          if List.for_all (fun id -> not (Lambda.IdentSet.mem id fv)) ids then
                ...
                Lsequence(
                  ...
                  Lprim(Pmakeblock(0, Immutable,Tr.no_type),
                        [mkappl (Lvar env_init, [lambda_unit]);
                         Lvar class_init; Lvar env_init; lambda_unit], None))))
          ...
          let make_envs lam =
            Llet(StrictOpt, envs,
                 (if linh_envs = [] then lenv else
                    Lprim(Pmakeblock(0, Immutable,Tr.no_type), lenv :: linh_envs, None)),
                 lam)
          and def_ids cla lam =
            Llet(StrictOpt, env2,
            ...
            if inh_keys = [] then Llet(Alias, cached, Lvar tables, lam) else
              Llet(Strict, cached,
                   mkappl (oo_prim "lookup_tables",
                           [Lvar tables; Lprim(Pmakeblock(0, Immutable,Tr.no_type), inh_keys, None)]),
                   lam)
          and lset cached i lam =
            Lprim(Psetfield(i, true), [Lvar cached; lam], None)
          in
          let ldirect () =
            ltable cla
