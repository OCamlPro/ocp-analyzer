...
module Make ( Tr: AltTypeTransducer.T ) (Matching : AltMatching.S with type ty = Tr.t)
...
struct
  ...
  let transl_module =
    ref((fun cc rootpath modl -> assert false) :
          module_coercion -> Path.t option -> module_expr -> Tr.t lambda)

  let transl_object =
    ref (fun id s cl -> assert false :
            Ident.t -> string list -> class_expr -> Tr.t lambda)
  ...
  let primitives_table = create_hashtable 57 [
      ...
      "%makeblock", Pmakeblock(0, Immutable, Tr.no_type);
      "%makemutable", Pmakeblock(0, Mutable, Tr.no_type);
      "%raise", Praise;
      "%sequand", Psequand;
      "%sequor", Psequor;
      "%boolnot", Pnot;
  ...
  let transl_primitive loc p =
    ...
    | _ ->
      ...
      Lfunction(Curried, params,
                Lprim(prim, List.map (fun id -> Lvar id) params, None))
  ...
  let check_recursive_lambda idlist lam =
    ...
    and check idlist = function
      ...
      | Lletrec(bindings, body) ->
        ...
        List.for_all (fun (id, arg) -> check idlist' arg) bindings &&
        check idlist' body
      | Lprim(Pmakeblock(tag, mut,_), args, _) ->
        List.for_all (check idlist) args
      | Lprim(Pmakearray(_,_), args, _) ->
        List.for_all (check idlist) args
      | Lsequence (lam1, lam2) -> check idlist lam1 && check idlist lam2
      | Levent (lam, _) -> check idlist lam
      | lam ->
        let fv = free_variables lam in
        not (List.exists (fun id -> Lambda.IdentSet.mem id fv) idlist)

    and add_let id arg idlist =
      let fv = free_variables arg in
      if List.exists (fun id -> Lambda.IdentSet.mem id fv) idlist
      then id :: idlist
      else idlist
  ...
  and transl_exp0 e =
    let tr_map_type () = Tr.map_type e.exp_env e.exp_type in
    match e.exp_desc with
    ...
    | Texp_apply({exp_desc = Texp_ident(path, _, {val_kind = Val_prim p})} as fn,
      ...
      let wrap f =
        ...
        else event_after e (transl_apply f args' e.exp_loc (Tr.map_type e.exp_type))
      in
      let wrap0 f =
      ...
      if public_send || p.prim_name = "%sendself" then
        ...
        | (_, _) ->
          begin match (prim, argl) with
            ...
            | _ ->
               let tr = if args' = [] then Some (Tr.map_type e.exp_type)
                 else None
               in
               let p = Lprim(prim, argl, tr) in
              if primitive_is_ccall prim then wrap p else wrap0 p
          end
      end
    | Texp_apply(funct, oargs) ->
       event_after e (transl_apply
                        (transl_exp funct)
                        oargs e.exp_loc
                        (Tr.map_type e.exp_type))
    ...
    | Texp_tuple el ->
      ...
      begin try
        ...
        with Not_constant ->
          Lprim(Pmakeblock(0, Immutable,tr_map_type ()), ll, None)
      end
      ...
      begin match cstr.cstr_tag with
        ...
        | Cstr_block n ->
          begin try
            ...
            with Not_constant ->
              Lprim(Pmakeblock(n, Immutable,tr_map_type ()), ll, None)
          end
        | Cstr_exception (path, _) ->
          Lprim(Pmakeblock(0, Immutable,tr_map_type ()), transl_path path :: ll, None)
      end
    | Texp_variant(l, arg) ->
      ...
      begin match arg with
        ...
        | Some arg ->
          ...
          with Not_constant ->
            Lprim(Pmakeblock(0, Immutable,tr_map_type ()),
    ...
    | Texp_setfield(arg, _, lbl, newval) ->
      ...
      Lprim(access, [transl_exp arg; transl_exp newval], None)
    | Texp_array expr_list ->
      ...
      begin try
        ...
        with Not_constant ->
          Lprim(Pmakearray (kind,tr_map_type ()), ll, None)
      end
    ...
    | Texp_send(expr, met, None) ->
      ...
      event_after e lam
    | Texp_new (cl, _, _) ->
      Lapply(Lprim(Pfield 0, [transl_path cl], None), [lambda_unit], Location.none, Tr.no_type)
    | Texp_instvar(path_self, path, _) ->
      Lprim(Parrayrefu Paddrarray, [transl_path path_self; transl_path path], None)
    ...
    | Texp_lazy e ->
      ...
      begin match e.exp_desc with
        ...
        | Texp_constant(Const_float _) ->
          Lprim(Pmakeblock(Obj.forward_tag, Immutable,tr_map_type ()), [transl_exp e], None)
        ...
        | _ ->
          ...
          Lprim(Pmakeblock(Config.lazy_tag, Immutable,tr_map_type ()), [fn], None)
      end
  ...
  and transl_apply lam sargs loc tr =
    let lapply funct args =
      ...
      | Levent(Lsend(k, lmet, lobj, largs, loc), _) ->
        Lsend(k, lmet, lobj, largs @ args, loc)
      | Lapply(lexp, largs, _, tr) ->
        Lapply(lexp, largs @ args, loc, tr)
      | lexp ->
        Lapply(lexp, args, loc, tr)
    in
  ...
  and transl_setinstvar self var expr =
    Lprim(Parraysetu (if maybe_pointer expr then Paddrarray else Pintarray),
          [self; transl_path var; transl_exp expr], None)
  ...
  and transl_record all_labels repres lbl_expr_list opt_init_expr =
    ...
    then begin
      ...
      let lam =
        ...
        with Not_constant ->
          match repres with
            Record_regular -> Lprim(Pmakeblock(0, mut,Tr.no_type), ll, None)
          | Record_float -> Lprim(Pmakearray (Pfloatarray,Tr.no_type), ll, None) in
      begin match opt_init_expr with
          None -> lam
      ...
      let update_field (_, lbl, expr) cont =
        let upd =
          ...
          | Record_float -> Psetfloatfield lbl.lbl_pos in
        Lsequence(Lprim(upd, [Lvar copy_id; transl_exp expr], None), cont) in
      begin match opt_init_expr with
          None -> assert false
        | Some init_expr ->
          Llet(Strict, copy_id,
...
*)
  ...
  let transl_exception id path decl =
    ...
    Lprim(Pmakeblock(0, Immutable,Tr.no_type), [Lconst(Const_base(Const_string name))], None)
