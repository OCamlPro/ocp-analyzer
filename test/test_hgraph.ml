...
module Env = struct
  ...
  let (!!) env = Envs.(!!) (env.env)
  
  let get_union ids env =
    fold__locs (fun _ v acc -> union v acc) ids bottom !!env
  ...
  let set_env id data env =
    env_v (set_data id data !!env)

  let set_data i data env =
    env_v (set_data i data !!env)

  let get_ident id env = get_ident id !!env
